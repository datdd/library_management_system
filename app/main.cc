...
struct AppServices {
    ...
    std::shared_ptr<lms::persistence_service::CachingFilePersistenceService> caching_file_persistence_service; // Might be null
...
void printHelp();
std::vector<std::string> readCommandArgs(const std::string& line); // Assuming this is defined as before
std::string availabilityStatusToString(lms::domain_core::AvailabilityStatus status); // Assuming defined
...
std::string availabilityStatusToString(lms::domain_core::AvailabilityStatus status) {
    ...
    switch (status) {
        case AS::AVAILABLE: return "Available";
        case AS::BORROWED: return "Borrowed";
        case AS::RESERVED: return "Reserved";
        case AS::MAINTENANCE: return "Maintenance";
...
void printHelp() {
    ...
    std::cout << "  addBook <item_id> \"<title>\" <author_id> \"<author_name>\" <isbn> <year>" << std::endl;
...
bool initializeServices(AppServices& services) {
    ...
    try {
        switch (choice) {
            case 1:
                ...
                services.persistence_service_interface = std::make_shared<lms::persistence_service::InMemoryPersistenceService>();
                break;
            case 2:
                ...
                services.persistence_service_interface = std::make_shared<lms::persistence_service::FilePersistenceService>(data_path, services.date_time_utils);
                break;
            case 3:
                std::cout << "Using Caching File-Based Persistence (operates in memory, loads/saves to '" << data_path << "')." << std::endl;
                if (!std::filesystem::exists(data_path)) {
                    std::filesystem::create_directories(data_path);
                }
                services.caching_file_persistence_service = std::make_shared<lms::persistence_service::CachingFilePersistenceService>(data_path, services.date_time_utils);
            ...
            case 4:
                {
                    ...
                    services.persistence_service_interface = std::make_shared<lms::persistence_service::MsSqlPersistenceService>(sql_conn_str, services.date_time_utils);
    ...
    services.user_service = std::make_shared<lms::user_service::UserService>(services.persistence_service_interface);
    services.catalog_service = std::make_shared<lms::catalog_service::CatalogService>(services.persistence_service_interface);
    services.notification_service = std::make_shared<lms::notification_service::ConsoleNotificationService>();
    ...
    try {
        if (command == "exit") {
            ...
            if (services.caching_file_persistence_service) {
                ...
                std::cout << "The 'saveAll' command is only available with Caching File-Based persistence." << std::endl;
            }
            ...
            if (user_opt) {
                std::cout << "User found: ID=" << user_opt->getUserId() << ", Name=" << user_opt->getName() << std::endl;
            } else {
                ...
                for (const auto& item_ptr : items) {
                    if (item_ptr) {
                         std::cout << "  ID: " << item_ptr->getId() << ", Title: " << item_ptr->getTitle()
                                   << ", Status: " << availabilityStatusToString(item_ptr->getAvailabilityStatus());
            ...
            std::cout << "  Loan ID: " << loan.getRecordId() 
                      << ", Due Date: " << services.date_time_utils->formatDate(loan.getDueDate()) << std::endl;
             ...
             if (loans.empty()) {
                ...
                for (const auto& loan : loans) {
                    ...
                    if(loan.getReturnDate().has_value()){
                        std::cout << ", Returned: " << services.date_time_utils->formatDate(loan.getReturnDate().value());
                    } else {
